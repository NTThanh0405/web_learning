...
function handleCourseActions($user) {
    ...
    } elseif ($action === 'add_student' && isset($_GET['course_id']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
        $courseId = $_GET['course_id'];
        ...
        if (empty($studentId)) {
            ...
            try {
                $stmt = $conn->prepare("
                    ...
                    VALUES (:course_id, :student_id, 'approved')
                    ON DUPLICATE KEY UPDATE status = 'approved'
                ");
                $stmt->execute(['course_id' => $courseId, 'student_id' => $studentId]);
                $success = true;
            } catch (PDOException $e) {
                $errors[] = "Lỗi khi thêm sinh viên: " . $e->getMessage();
            }
        }
...
<div class="container mt-5">
    ...
    <?php elseif ($action === 'add_student' && isset($_GET['course_id'])): ?>
        <h2>Thêm Sinh Viên Vào Khóa Học</h2>
        <?php if ($success): ?>
            <div class="alert alert-success">
        ...
        <form method="POST">
            <div class="form-group">
                <label for="student_search">Tìm sinh viên:</label>
                <input type="text" id="student_search" class="form-control" placeholder="Gõ tên sinh viên để tìm kiếm" autocomplete="off">
                <input type="hidden" name="student_id" id="student_id">
                <div id="student_suggestions" class="list-group" style="position: absolute; z-index: 1000; width: 100%; max-height: 200px; overflow-y: auto;"></div>
            </div>
    ...
    <?php elseif ($action === 'view' && isset($_GET['course_id'])): ?>
        <?php if ($course): ?>
            ...
            <?php endif; ?>
            <a href="<?php echo BASE_URL; ?>teacher/index.php?tab=courses&action=add_student&course_id=<?php echo $course['id']; ?>" class="btn btn-success mb-3">Thêm sinh viên</a>
    ...
    $('#student_search').on('input', function() {
        ...
        if (query.length >= 1) {
            debounceTimer = setTimeout(function() {
                $.ajax({
                    url: '<?php echo BASE_URL; ?>search.php',
                    method: 'POST',
                    data: { q: query, type: 'students', course_id: courseId },
                    dataType: 'json',
                    success: function(data) {
                        var suggestions = '';
                        if (data.length > 0) {
                            data.forEach(function(student) {
                                suggestions += '<a href="#" class="list-group-item list-group-item-action student-suggestion" data-id="' + student.id + '">' + student.full_name + '</a>';
                            });
                        } else {
                            suggestions = '<div class="list-group-item">Không tìm thấy sinh viên phù hợp</div>';
                        }
                        $('#student_suggestions').html(suggestions).show();
                    },
                    error: function(xhr, status, error) {
                        console.error('Lỗi AJAX: ' + status + ' - ' + error);
                        $('#student_suggestions').html('<div class="list-group-item">Có lỗi xảy ra khi tìm kiếm</div>').show();
                    }
                });
            }, 300);
        } else {
