...
try {
    ...
    if ($action === 'enroll') {
        ...
        if (!$course_id || !$user_id) {
            $response['message'] = 'Thiếu thông tin khóa học hoặc người dùng.';
        } else {
            ...
            if (!$stmt->fetch()) {
                ...
                if (!$user || $user['id'] != $user_id || $user['role'] !== 'student') {
                    ...
                    // Ghi danh hoặc cập nhật trạng thái
                    $stmt = $conn->prepare("
                        INSERT INTO course_enrollments (course_id, student_id, status, created_at)
