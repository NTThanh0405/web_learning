...
switch ($action) {
    ...
    case 'create_group_notification':
        if ($user['role'] !== 'teacher') {
            echo json_encode(['status' => 'error', 'message' => 'Only teachers can create group notifications']);
            exit;
        }
        ...
        $conn = getDBConnection();
        $stmt = $conn->prepare("SELECT id FROM groups WHERE id = :group_id AND creator_id = :user_id");
        $stmt->execute(['group_id' => $group_id, 'user_id' => $user['id']]);
        if (!$stmt->fetch()) {
            echo json_encode(['status' => 'error', 'message' => 'Invalid group or unauthorized']);
            $conn = null;
            exit;
        }
        $conn = null;
        ...
        try {
            createGroupNotification($group_id, $user['id'], $title, $content, $scope);
            echo json_encode(['status' => 'success', 'message' => 'Group notification created']);
        } catch (Exception $e) {
