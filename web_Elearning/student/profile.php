...
// Lấy tổng số khóa học mà sinh viên đã tham gia (status = 'approved')
try {
    $stmt = $conn->prepare("SELECT COUNT(*) as enrolled_count 
                            FROM course_enrollments 
                            WHERE student_id = :student_id AND status = 'approved'");
    $stmt->execute(['student_id' => $user['id']]);
    $enrolledCount = $stmt->fetch(PDO::FETCH_ASSOC)['enrolled_count'];
...
<div class="container mt-5">
    ...
    <div class="card mb-4">
        <div class="card-body text-center">
            ...
            <p><strong>Tổng số khóa học đã tham gia:</strong> <?php echo $enrolledCount; ?></p>
        </div>
    </div>
