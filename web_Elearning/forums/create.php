...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_post'])) {
    ...
    // Kiểm tra quyền xóa (chỉ người tạo hoặc giáo viên)
    $stmt = $conn->prepare("SELECT user_id FROM forum_posts WHERE id = :post_id");
    $stmt->execute(['post_id' => $post_id]);
    $post = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($post && ($post['user_id'] == $user['id'] || $user['role'] === 'teacher')) {
        try {
...
// Xác định post_id từ query string
...
<body>
    <div class="container mt-4">
        ...
        <!-- Danh sách bài đăng -->
        <?php if (empty($posts)): ?>
            <p>Chưa có bài đăng nào trong diễn đàn.</p>
        <?php else: ?>
                ...
                <div class="post-container">
                    ...
                    <div class="post-content">
                        ...
                        <div class="post-actions">
                            ...
                            <?php if ($post['user_id'] == $user['id'] || $user['role'] === 'teacher'): ?>
                                ...
                                ?>
                                    <div class="d-flex align-items-center mb-2 parent-comment" data-comment-id="<?php echo $comment['id']; ?>">
                                        ...
                                        <div class="ml-2">
                                            ...
                                            <p><?php echo nl2br(htmlspecialchars($comment['comment'])); ?></p> <!-- Sửa 'content' thành 'comment' -->
...
$(document).ready(function() {
    // Mở modal khi nhấn nút tạo bài đăng
...
$(document).on('click', '.show-child-comments', function(e) {
    ...
    var postId = $(this).closest('.post-container').find('.comment-form').data('post-id'); // Lấy postId từ comment-form
    ...
    if ($childComments.is(':visible')) {
        ...
        // Tải lại tất cả bình luận con
        $.ajax({
            url: '<?php echo BASE_URL; ?>api/comments.php',
            method: 'GET',
            data: { 
                parent_comment_id: commentId, 
                post_id: postId, // Gửi post_id
    ...
    // Xử lý gửi form bình luận qua AJAX
    $(document).on('submit', '.comment-form', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    // Làm mới danh sách bình luận cha
                    $.ajax({
                        url: '<?php echo BASE_URL; ?>api/comments.php',
                        method: 'GET',
                        data: { post_id: postId, action: 'get_all_parent_comments' },
